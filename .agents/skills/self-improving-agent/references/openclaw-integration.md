...
Keep `.learnings/` at the workspace root, not inside the installed skill
directory. The skill should capture repo- or workspace-local lessons, not
global hidden state.

## Hook Guard

The optional bootstrap hook should stay quiet unless one of these is true:

- the active workspace already contains `.learnings/`
- `SELF_IMPROVING_AGENT_FORCE=1` is set intentionally for testing

Do not enable a noisy global reminder flow across unrelated workspaces unless
...
Some OpenClaw environments provide tools for cross-session communication. Verify
availability in the actual runtime before depending on these names:
