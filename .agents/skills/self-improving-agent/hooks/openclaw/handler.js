...
function collectCandidateRoots(event) {
  const maybeRoots = [
    event.cwd,
    event.workspacePath,
    event.context?.cwd,
  ...
  return [...new Set(maybeRoots.filter((value) => typeof value === 'string' && value.trim().length > 0))];
}
...
const handler = async (event) => {
  ...
  // Skip sub-agent sessions to avoid bootstrap noise in delegated work.
