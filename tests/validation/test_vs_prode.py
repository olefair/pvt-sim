"""Optional live cross-checks against a Prode-backed validation bridge.

This lane is intentionally secondary and optional:

- it never participates in the runtime solver path
- it is skipped entirely unless both a case file and a working Prode bridge
  are available
...
from pvtcore.validation import detect_prode_validation_backend, load_prode_validation_backend


_CASES_DIR = Path(__file__).resolve().parent / "prode" / "cases"
...
def _assert_curve_alignment(
    ...
    *,
    ...
    assert comparable > 0, "No Prode envelope points overlapped the simulated branch temperature range."
...
class ProdeCase:
    ...
    trace: Dict[str, Any]
    prode_options: Dict[str, Any]


def _load_cases() -> List[ProdeCase]:
    if not _CASES_DIR.exists():
        return []

    cases: List[ProdeCase] = []
    for path in sorted(_CASES_DIR.glob("*.json")):
        ...
        cases.append(
            ProdeCase(
                ...
                prode_options=obj.get("prode_options", {}) or {},
            )
        )
    return cases
...
if not _CASES:
    pytest.skip(
        f"No Prode validation cases found at: {_CASES_DIR}. Add *.json case files to enable Prode comparison tests.",
        allow_module_level=True,
    )

_PRODE_AVAILABILITY = detect_prode_validation_backend()
if not _PRODE_AVAILABILITY.available:
    pytest.skip(
        f"Prode validation bridge unavailable: {_PRODE_AVAILABILITY.reason}",
        allow_module_level=True,
    )

_PRODE_BACKEND = load_prode_validation_backend()


@pytest.mark.parametrize("case", _CASES, ids=lambda c: f"{c.case_id}:{c.task}")
def test_vs_prode(case: ProdeCase) -> None:
    ...
    if case.task == "pt_flash":
        ...
        expected = _PRODE_BACKEND.pt_flash(
    ...
    if case.task == "bubble_point":
        ...
        expected = _PRODE_BACKEND.bubble_point(
            temperature_k=case.temperature_k,
            component_ids=case.component_ids,
            composition=case.z.tolist(),
            eos_name="Peng-Robinson",
    ...
    if case.task == "dew_point":
        ...
        expected = _PRODE_BACKEND.dew_point(
            temperature_k=case.temperature_k,
            component_ids=case.component_ids,
            composition=case.z.tolist(),
            eos_name="Peng-Robinson",
    ...
    if case.task == "phase_envelope":
        ...
        expected = _PRODE_BACKEND.phase_envelope(
            component_ids=case.component_ids,
            composition=case.z.tolist(),
            eos_name="Peng-Robinson",
    ...
    raise ValueError(f"Unknown Prode case task: {case.task!r}")
